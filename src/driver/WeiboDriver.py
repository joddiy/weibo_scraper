...
# ------------------------------------------------------------------------
from scraper.scraper_main import WeiBoSearchScraper
from scraper.weibo_scraper import WeiBoScraper
from config.config import COOKIES_SAVE_PATH
...
class Dispatcher(object):
    ...
    def _init_accounts_cookies(self):
        ...
        :return:
        """
        if self.update_cookies or not os.path.exists(COOKIES_SAVE_PATH):
    ...
    def _init_search_mode(self):
        scraper = WeiBoSearchScraper(using_account=self.all_accounts[0], uuid=self.user_id, filter_flag=self.filter_flag)
        i = 1
        while True:
            result = scraper.crawl()
            if result:
